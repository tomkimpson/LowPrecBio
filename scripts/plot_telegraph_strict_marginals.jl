...
"""
Generate publication-quality telegraph strict-mode marginal distribution figure
using CairoMakie.

Shows FP64 baseline + 4 strict-precision modes overlaid. Y-axis truncated to
show tail structure; P(n=0) annotated.

Usage:
  julia --project=. scripts/plot_telegraph_strict_marginals.jl
  julia --project=. scripts/plot_telegraph_strict_marginals.jl \
    --input-jld2=results/telegraph/ensemble_data.jld2 \
    --output-stem=figures/telegraph_marginals_strict
"""
...
function main()
    ...
    output_stem = arg_string(opts, "output-stem", "figures/telegraph_marginals_strict")

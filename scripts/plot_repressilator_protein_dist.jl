...
"""
Generate publication-quality repressilator protein distribution figure using CairoMakie.

Single-panel stairs histogram showing all 8 modes (6 mixed + 2 strict).

Usage:
  julia --project=. scripts/plot_repressilator_protein_dist.jl
  julia --project=. scripts/plot_repressilator_protein_dist.jl \
    --input-jld2=results/repressilator/ensemble_data.jld2 \
    --output-stem=figures/repressilator_protein_dist
"""
...
function main()
    ...
    output_stem = arg_string(opts, "output-stem", "figures/repressilator_protein_dist")
    ...
    # All 8 modes: 6 mixed + 2 strict SR (STRICT BF16+SR last so shifted peak overlays)
    labels = [
        "FP64 baseline",
        "FP32",
        "BF16 + SR",
    ...
    counts_all = [ensemble[lbl]["end_pA"] for lbl in labels]

    # Shared histogram edges — use wider bins for smoother appearance
    ...
    linewidths = [2.5, 2.0, 1.8, 1.5, 1.3, 1.0, 1.5, 2.0]
